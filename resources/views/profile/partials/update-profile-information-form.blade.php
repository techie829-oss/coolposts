    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <select id="currency" name="currency" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                <option value="INR" {{ old('currency', $user->currency ?? 'INR') === 'INR' ? 'selected' : '' }}>
                    ₹ Indian Rupee (INR)
                </option>
                <option value="USD" {{ old('currency', $user->currency ?? 'INR') === 'USD' ? 'selected' : '' }}>
                    $ US Dollar (USD)
                </option>
            </select>
            <p class="mt-1 text-sm text-gray-500">
                {{ __('This will be your default currency for earnings and payouts.') }}
            </p>
