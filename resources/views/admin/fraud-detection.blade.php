                            ...
                            <div class="p-6">
                                <div class="space-y-4">
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span class="font-semibold text-gray-900">{{ $settings['auto_block'] ?? 'Enabled' }}</span>
                                    </div>
                                    <div class="flex justify-between items-center">
                                        <span class="text-gray-600">Rate limiting</span>
                                        <span class="font-semibold text-gray-900">{{ $settings['rate_limiting'] ?? 'Enabled' }}</span>
                                    </div>
                                    <div class="flex justify-between items-center">
                                        <span class="text-gray-600">Geolocation blocking</span>
                                        <span class="font-semibold text-gray-900">{{ $settings['geo_blocking'] ?? 'Disabled' }}</span>
                                    </div>
                                    <div class="flex justify-between items-center">
                                        <span class="text-gray-600">Bot detection</span>
                                        <span class="font-semibold text-gray-900">{{ $settings['bot_detection'] ?? 'Enabled' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    @forelse($recent_alerts ?? [] as $alert)
                                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500">{{ $alert['ip'] }} - {{ $alert['time'] }}</p>
                                            </div>
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $alert['severity'] === 'high' ? 'bg-red-100 text-red-800' : ($alert['severity'] === 'medium' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800') }}">
                        ...
                        <div class="p-6">
                            <div class="mb-4">
                                <form class="flex gap-2">
                                    <input type="text" placeholder="Enter IP address" class="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Block IP</button>
                                        ...
                                        @forelse($ip_list ?? [] as $ip)
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $ip['status'] === 'blocked' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
                                                        {{ ucfirst($ip['status']) }}
                                                    </span>
                                                </td>
