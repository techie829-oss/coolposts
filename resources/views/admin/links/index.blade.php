                    ...
                    <form method="GET" action="{{ route('admin.links.index') }}" class="flex flex-wrap gap-4">
                        ...
                        <div class="flex gap-2">
                            <select name="ad_type"
                                ...
                                <option value="no_ads" {{ request('ad_type') === 'no_ads' ? 'selected' : '' }}>No Ads
                                </option>
                                <option value="short_ads" {{ request('ad_type') === 'short_ads' ? 'selected' : '' }}>
                                    Short Ads</option>
                                <option value="long_ads" {{ request('ad_type') === 'long_ads' ? 'selected' : '' }}>Long
                                    Ads</option>
                            </select>
                            <select name="status"
                                    ...
                                    @foreach ($links as $link)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <div class="flex-shrink-0 h-10 w-10">
                                                        <div
                                                            class="h-10 w-10 rounded-full bg-gradient-to-r from-green-500 to-blue-600 flex items-center justify-center text-white">
                                                            <i class="fas fa-link"></i>
                                                        </div>
                                                    </div>
                                                    <div class="ml-4">
                                                        <div class="text-sm font-medium text-gray-900">
                                                            {{ Str::limit($link->title, 30) }}</div>
                                                        <div class="text-sm text-gray-500">
                                                            {{ Str::limit($link->short_url, 40) }}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                ...
                                                <div class="text-sm text-gray-500">
                                                    {{ $link->unique_clicks_count ?? 0 }} unique</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span
